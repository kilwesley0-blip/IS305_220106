 ...
 * ID#: 220106
 * DiningApp.js
 * Part 2: Interactive console application for DWU Dining Meal Booking.
...
async function createBooking() {
  ...
  try {
    ...
    // Duplicate check
    if (isDuplicate(studentId.trim(), mealDate.trim(), mealType.trim())) {
      console.log(
        `\n[ERROR] Duplicate booking detected! A booking already exists for ` +
          `student ${studentId.trim()} on ${mealDate.trim()} for ${mealType.trim()}.\n`
      );
      return;
    }
...
async function runTests() {
  ...
  /* --- Test 2: Invalid booking (missing name, invalid meal type, quantity 0) --- */
